...
const CreateSharedRideScreen = ({ navigation }) => {
  ...
  const [scheduledTime, setScheduledTime] = useState(new Date(Date.now() + 7*  60 * 60 * 1000)); // +1 hour
  ...
  const loadVehicles = async () => {
    try {
      ...
      if (error.message?.includes('Driver profile not found')) {
        ...
        errorMessage = 'Bạn chưa đăng ký phương tiện nào. Vui lòng thêm phương tiện trước.';
      }
  ...
  const handleCreateRide = async () => {
    ...
    try {
      ...
      if (!processedStartLocation && startAddress.trim()) {
        ...
        try {
          const geocodeResults = await goongService.geocode(startAddress.trim());
      ...
      if (!processedStartLocation || !processedEndLocation) {
        Alert.alert('Lỗi', 'Không thể xác định tọa độ cho địa chỉ đã nhập. Vui lòng chọn từ danh sách gợi ý hoặc nhập địa chỉ chính xác hơn.');
        return;
      }
      ...
      const rideData = {
        ...
        scheduledDepartureTime: '2025-10-19T18:26:16.067Z',
      };
      ...
      Alert.alert(
        'Thành công!',
        `Đã tạo chuyến đi chia sẻ #${result.sharedRideId || result.id}`,
        [
          {
            text: 'Xem chi tiết',
            onPress: () => {
          ...
          },
          {
            ...
            onPress: () => {
              ...
              setStartAddress('');
              setEndAddress('');
              // setScheduledTime(new Date(Date.now() + 7 * 60 * 60 * 1000));
              '2025-10-19T18:26:16.067Z'
            }
          }
        ]
      );
          ...
          {loadingVehicles ? (
            ...
            <TouchableOpacity 
              ...
              onPress={() => {
                ...
                Alert.alert('Chọn phương tiện', 'Chức năng chọn phương tiện sẽ được cập nhật sớm');
              }}
            >
              <Icon name="motorcycle" size={24} color="#4CAF50" />
              <View style={styles.vehicleInfo}>
                <Text style={styles.vehicleName}>{vehicles[0]?.displayName || vehicles[0]?.model}</Text>
                <Text style={styles.vehiclePlate}>{vehicles[0]?.plateNumber}</Text>
                <Text style={styles.vehicleStatus}>
                  {vehicles[0]?.isVerified ? '✓ Đã xác minh' : '⚠ Chưa xác minh'}
                </Text>
              </View>
